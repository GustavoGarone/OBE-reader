...
class CornerCropper:
    ...
    def _write_row(self, p: Path, row: List[float]) -> None:
        ...
        p.write_text(
            " ".join(f"{int(x)}" if i == 0 else f"{x:.6f}" for i, x in enumerate(row))
            + "\n"
        )
    ...
    def _flip_img_to_tl(self, img: np.ndarray, cid: int) -> np.ndarray:
        if cid == 0:
            return img
        if cid == 1:
            return cv2.flip(img, 1)  # H
        if cid == 2:
            return cv2.flip(img, 0)  # V
        if cid == 3:
            return cv2.flip(img, -1)  # H+V
        raise ValueError
    ...
    def split_page(
        self, page: ImgAnn
    ) -> Tuple[List[np.ndarray], List[List[float]], List[CropMeta]]:
        ...
        for cid, (x, y) in enumerate(kpxy):
            ...
            metas.append(
                CropMeta(
                    ...
                    cw=cw,
                    ch=ch,
                    crop_name=f"{page.img_path.stem}_kp{cid}",
                )
            )
        return crops, rows, metas

    def merge_page(
        self, meta_list: List[CropMeta], crop_rows: List[List[float]]
    ) -> List[float]:
        ...
        pad = 0.001
        x1, y1 = np.clip(k.min(0) - pad, 0, 1)
        x2, y2 = np.clip(k.max(0) + pad, 0, 1)
        xc, yc = (x1 + x2) / 2, (y1 + y2) / 2
        bw, bh = (x2 - x1), (y2 - y1)
        klist = []
        for x, y in k:
            klist += [float(x), float(y), 2.0]
...
def build_crops(data_yaml: Path, out_root: Path, cfg: CropCfg = CropCfg()) -> None:
    ...
    out_lbl_tr = out_root / "labels" / "train"
    out_lbl_va = out_root / "labels" / "val"
    for p in [out_img_tr, out_img_va, out_lbl_tr, out_lbl_va]:
...
def crop_images(
    images: List[Path], cropper: CornerCropper, tmp_root: Path, factor: float
) -> List[CropMeta]:
    metas: List[CropMeta] = []
    for ip in images:
        bgr = cv2.imread(str(ip), cv2.IMREAD_COLOR)
        ...
        for cid, (x0, y0) in corners.items():
            ...
            (tmp_root / "labels" / f"{stem}.meta.json").write_text(
                json.dumps(meta.__dict__, ensure_ascii=False)
            )
            metas.append(meta)
    return metas
...
def _row_to_page_xy(
    meta: CropMeta, row: List[float], cropper: CornerCropper
) -> Tuple[float, float]:
...
def _infer_fourth(
    pts4: List[Tuple[float, float] | None],
    ...
    if m == 0:
        P[0] = sub(add(TR, BL), BR)
    elif m == 1:
        P[1] = sub(add(TL, BR), BL)
    elif m == 2:
        P[2] = sub(add(TL, BR), TR)
    else:
        P[3] = sub(add(TR, BL), TL)
    return P, m


def _row_from_points(
    pts4: List[Tuple[float, float]], W: int, H: int, cls_id: int = 0
) -> List[float]:
...
def fallback_logic(
    ...
    missing = [
        i for i, (m, r) in enumerate(zip(metas4, rows4)) if (m is None or r is None)
    ]
    if not missing:
    ...
    if len(missing) == 1:
        ...
        for cid in range(4):
            ...
            if m is None or r is None:
                ...
                pts[cid] = _row_to_page_xy(m, r, cropper)
...
def merge_images(
    ...
    for page_rel, pack in sorted(by_page.items(), key=lambda kv: kv[0]):
        ...
        if row is None:
            ...
            miss = [
                i
                for i, (m, r) in enumerate(zip(metas4, rows4))
                if (m is None or r is None)
            ]
...
def run_corner_pose(
    ...
    merged, filled, partial = merge_images(
        by_page, d["out_img"], d["out_lbl"], d["out_unl"], cropper
    )
    write_data_yaml(out_root)
    shutil.rmtree(tmp_root)
    print(
        f"[DONE] merged_pages={merged}, filled_pages={filled}, partial_pages={partial}, out_root={out_root}"
    )
