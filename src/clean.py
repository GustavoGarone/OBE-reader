...
import io, imghdr, base64, shutil, re, cv2
import numpy as np
...
def triage_to_jpg(src_dir: str | Path, dst_dir: str | Path, size: Tuple[int, int]) -> None:
            ...
            try:
                for i, page in enumerate(doc):
                    ...
                    with Image.open(io.BytesIO(pix.tobytes("png"))) as im:
                        name = f"{stem}_p{i+1}.jpg" if doc.page_count > 1 else f"{stem}.jpg"
                        save(im, dst / name)
            finally:
                doc.close()
        else:
...
def _black_ratio_on_canvas(img: Image.Image, eval_size: Tuple[int, int], border: int = 0) -> float:
    """Letterbox to eval_size on black, then return fraction of pure-black pixels."""
    canvas = ImageOps.pad(img.convert("RGB"), eval_size, method=Image.BICUBIC, color=(0, 0, 0))
    if border > 0:
        w, h = canvas.size
...
def scan_black_ratios_ar(src_jpg_dir: str | Path, eval_size: Tuple[int, int], border: int = 0) -> List[tuple[str, float]]:
...
def _zoom_and_resize(im: Image.Image, out_size: Tuple[int, int], tol: int = 3, margin_frac: float = 0.01) -> Image.Image:
...
def _circle_annulus_contrast(gray: np.ndarray, x: int, y: int, r: int) -> tuple[float, float]:
...
def detect_filled_bubbles(gray: np.ndarray) -> tuple[np.ndarray, List[tuple[int, int, int]]]:
    ...
    circles = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, dp=1.2, minDist=max(6, rmin),
                               param1=160, param2=18, minRadius=rmin, maxRadius=rmax)
    if circles is None:
    ...
    circles2 = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, dp=1.2, minDist=min_dist,
                                param1=160, param2=20, minRadius=int(0.8*r_med), maxRadius=int(1.2*r_med))
...
def classify_dir(src_dir: str | Path, out_root: str | Path, save_masks: bool = False) -> None:
...
def debug_show(path: str | Path) -> None:
    ...
    mask, kept = detect_filled_bubbles(gray)
    print(("IS FIRST PAGE" if len(kept) > MIN_BUBBLES_FIRST else "NOT FIRST PAGE")
          + f" | bubbles={len(kept)}")
