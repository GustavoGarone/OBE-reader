import csv, argparse, sys, atexit, select
...
def parse_list_allow_dash(s: str) -> List[int]:
    ...
    for token in s.replace(",", " ").split():
        ...
        if token == "-": out.append(-1); continue
        try: out.append(int(token))
        except ValueError: raise ValueError(f"invalid token '{token}' (use digits or '-')")
    return out
...
def parse_answers_tokens(s: str) -> List[int]:
    ...
    for token in s.replace(",", " ").split():
        ...
        if token == "-": out.append(-1); continue
        low = token.lower()
        if low in LETTER2CHOICE: out.append(LETTER2CHOICE[low]); continue
        try: out.append(int(token))
        except ValueError: raise ValueError(f"invalid answers token '{token}' (use a/b/c/d, digits, or '-')")
    return out
...
def image_iter(images_arg: Path, recursive: bool, pattern: Optional[str]) -> Iterable[Path]:
    base = images_arg
    if base.is_file():
...
def make_mapped_report_path(img_path: Path, images_root: Path, report_root: Optional[Path]) -> Path:
...
class Viewer:
    ...
    def _on_skip_clicked(self, event=None): self.skip_flag = True
    ...
    def show(self, display_path: Path, row_idx: int, sid: Sequence[int], ans: Sequence[int], mode: str) -> None:
        try:
        ...
        if self.img_artist is None: self.img_artist = self.ax.imshow(im)
        else: self.img_artist.set_data(im)

        if self.text_artist is not None: self.text_artist.remove()
        ...
        self.text_artist = self.ax.text(
            5, 5, text, transform=self.ax.transAxes, va="top", ha="left",
            bbox=dict(facecolor="white", alpha=0.7, edgecolor="black", pad=5), fontsize=10
        )
        self.fig.canvas.manager.set_window_title(f"[{mode}] {row_idx} — {display_path.name}")
        self.fig.canvas.draw_idle()
        plt.pause(0.001)

    def ping(self): plt.pause(0.001)
    def close(self):
...
def typed(prompt: str, viewer: Viewer) -> str:
    ...
    while True:
        ...
        try:
            ...
            print("\n[interrupt] quitting now."); raise

def run_fix_mode(csv_path: Path, lo_id: int, hi_id: int, lo_an: int, hi_an: int, jump: int) -> None:
    rows = read_csv_rows(csv_path)
    header = rows[0]
    try:
        i_sid = header.index("student_id"); i_ans = header.index("answers"); i_rep = header.index("report_path")
    except ValueError as e:
        print(f"missing expected column: {e}"); sys.exit(1)

    viewer = Viewer(); atexit.register(viewer.close)
    ...
    try:
        while i < len(rows):
            ...
            if viewer.consume_skip():
                print("[skip] skipping this row."); i += 1; continue

            s = typed("student_id = ", viewer).strip()
            if s == "__SKIP__": i += 1; continue
            if s.lower() == "q": break
            if s == "": sid_new = sid_cur
            else:
                try:
                    sid_new = parse_list_allow_dash(s)
                    if not all((x == -1) or (lo_id <= x <= hi_id) for x in sid_new): raise ValueError
                except Exception:
                    print(f"  invalid student_id; use digits [{lo_id}..{hi_id}] or '-' for -1."); continue
            sid_cur = sid_new; viewer.show(report_img_fs, i, sid_cur, ans_cur, mode="fix")
            ...
            if s == "__SKIP__": i += 1; continue
            if s.lower() == "q": break
            if s == "": ans_new = ans_cur
            else:
                try:
                    ans_new = parse_answers_tokens(s)
                    if not all((x == -1) or (lo_an <= x <= hi_an) for x in ans_new): raise ValueError
                except Exception as e:
                    print(f"  {e}"); print(f"  valid answers: a/b/c/d (→0/1/2/3), digits [{lo_an}..{hi_an}], or '-'"); continue
            ans_cur = ans_new; viewer.show(report_img_fs, i, sid_cur, ans_cur, mode="fix")
            ...
            if confirm == "__SKIP__": i += 1; continue
            if confirm == "y":
...
def run_append_mode(
    ...
    lo_id: int, hi_id: int, lo_an: int, hi_an: int,
    ...
    header = rows[0]
    try:
        i_sid = header.index("student_id"); i_ans = header.index("answers")
        i_img = header.index("img_path"); i_rep = header.index("report_path")
    except ValueError as e:
        print(f"missing expected column: {e}"); sys.exit(1)

    existing_pairs = {(to_rel_data_str(r[i_img]), to_rel_data_str(r[i_rep])) for r in rows[1:]}
    ...
    try:
        ...
        for img_path in image_iter(images, recursive, pattern):
            ...
            rep_real = make_mapped_report_path(img_real, images_root, rep_root_resolved).resolve()
            ...
            sid_cur: List[int] = []; ans_cur: List[int] = []
            ...
            print("student_id: digits; use '-' for missing (-1)")
            print("answers   : digits OR letters a/b/c/d; '-' for missing (-1)")
            print("ENTER keeps field (empty → []) | 'q' quits | Skip button skips image")

            if viewer.consume_skip():
                print("[skip] skipping this image."); idx += 1; continue

            s = typed("student_id = ", viewer).strip()
            if s == "__SKIP__": idx += 1; continue
            if s.lower() == "q": break
            if s == "": sid_new = sid_cur
            else:
                try:
                    ...
                    print(f"  invalid student_id; use digits [{lo_id}..{hi_id}] or '-' for -1."); continue
            sid_cur = sid_new; viewer.show(img_real, idx, sid_cur, ans_cur, mode="append")
            ...
            if s == "": ans_new = ans_cur
                ...
                try:
                    ...
                    print(f"  {e}"); print(f"  valid answers: a/b/c/d (→0/1/2/3), digits [{lo_an}..{hi_an}], or '-'"); continue
            ans_cur = ans_new; viewer.show(img_real, idx, sid_cur, ans_cur, mode="append")
            ...
            if confirm == "__SKIP__": idx += 1; continue
            if confirm == "y":
        ...
        viewer.close(); print(f"bye. appended {appended} rows.")
...
def main():
    ...
    ap.add_argument("--jump", type=int, default=0, help="[fix] Start from this row index (after header).")
    ap.add_argument("--id-range", type=str, default="0-9", help="Valid digit range for student_id (e.g., 0-9).")
    ap.add_argument("--ans-range", type=str, default="0-3", help="Valid choice range for answers (e.g., 0-3).")

    ap.add_argument("--images", type=Path, default=None,
                    help="[append] Directory, glob-list file (.txt), or single image to label.")
    ap.add_argument("--recursive", action="store_true", help="[append] Recurse subfolders when scanning a directory.")
    ...
    if args.images is None:
        ...
        run_append_mode(
            ...
            lo_id=lo_id, hi_id=hi_id, lo_an=lo_an, hi_an=hi_an,
